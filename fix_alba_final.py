#!/usr/bin/env python3
"""
Fix alba player placement - remove from inside model-viewer and place after it
"""
...
def fix_alba_placement(filepath):
    """Fix alba player placement"""
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Find alba player block inside model-viewer
    alba_pattern = r'(\s*)<!-- Alba Futuristic Audio Player -->.*?</div>\s*\n\s*</model-viewer>'
    
    match = re.search(alba_pattern, content, re.DOTALL)
    if not match:
        return False
    
    indent = match.group(1)
    
    # Extract the alba block
    alba_start = match.start()
    alba_content = content[alba_start:match.end()]
    
    # Remove alba from inside model-viewer
    content_without_alba = content[:alba_start] + '\n    </model-viewer>' + content[match.end():]
    
    # Find </model-viewer> in the new content and insert alba after it
    model_viewer_close = content_without_alba.find('</model-viewer>')
    if model_viewer_close != -1:
        insert_pos = model_viewer_close + len('</model-viewer>')
        # Add alba player after model-viewer (without the indent and comment part, clean it up)
        alba_html = '''
    ...
    <div id="albaPlayer">
      ...
      <div class="alba-status" id="albaStatus">Müzik seç ve dinle 🎵</div>
        ...
        final_content = content_without_alba[:insert_pos] + alba_html + content_without_alba[insert_pos:]
